...
const PremiumAbout = ({ content }: { content?: any }) => {
  return (
    <section className="py-16 md:py-32 w-full bg-[#0a0a0a] text-white font-sans px-6 md:px-12 lg:px-16 overflow-hidden relative border-t border-white/5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-y-16 md:gap-x-12 w-full max-w-[1400px] mx-auto relative z-10">
        ...
        <motion.div 
          ...
          className="md:col-span-4 lg:col-span-4 pt-4 border-t border-white/10"
        >
        ...
        <div className="md:col-span-8 lg:col-span-8 flex flex-col gap-24">
          ...
          >
            ...
            <h3 className="font-sans text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-white/40 mb-2">
            ...
            <div className="flex flex-col gap-8">
              ...
              <p className="font-sans text-lg md:text-2xl font-normal text-white/60 leading-relaxed tracking-tight max-w-2xl">
    ...
    >
      <div className="card-top-row">
        <Link href={`/properties/${property._id}`} className="id-brand-group group/title">
          <span className="huge-number text-white group-hover/title:text-primary transition-colors">{(index + 1).toString().padStart(3, '0')}</span>
          <div className="client-info">
            <span className="label text-white uppercase tracking-widest text-[10px] md:text-xs group-hover/title:text-primary transition-colors">{property.title}</span>
            <span className="client-name text-white/40 uppercase tracking-widest text-[8px] md:text-[10px]">{property.location}</span>
          </div>
        </Link>
      </div>
      ...
      <div className="content-grid">
        ...
        >
          ...
          <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover/img:opacity-100 transition-opacity duration-700 flex items-center justify-center">
            <span className="bg-black/60 backdrop-blur-md text-white px-6 py-3 rounded-full font-black uppercase tracking-widest text-[10px] border border-white/20">
              View Experience
            </span>
          </div>
        </Link>
        <div className="project-description">
          <div className="space-y-6">
             <div>
                ...
                <p className="text-4xl md:text-6xl font-black tracking-tighter text-white">₹{property.price.toLocaleString('en-IN')}</p>
             </div>
             ...
             <div className="flex gap-4 pt-4">
                <span className="px-4 py-2 border border-white/10 rounded-full text-[9px] font-black text-white/60 uppercase tracking-[0.2em]">{property.type}</span>
                <span className="px-4 py-2 border border-white/10 rounded-full text-[9px] font-black text-white/60 uppercase tracking-[0.2em]">Immediate Registration</span>
             </div>
          </div>
        </div>
      </div>
...
const PremiumHome = ({ properties, content }: PremiumHomeProps) => {
  ...
  return (
    <div className="min-h-screen relative bg-black selection:bg-[#10b981] selection:text-white font-sans">
      ...
      <div id="properties" className="relative z-20 bg-black pt-10 md:pt-20">
         <div className="w-full h-[15vh] md:h-[25vh] lg:h-[35vh] overflow-hidden flex items-center relative z-10 bg-black border-y border-white/5 shadow-2xl">
        ...
        <motion.div style={{ y: isMobile ? 0 : footerY }} className="text-center px-6 w-full max-w-6xl">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 text-center md:text-left border-t border-white/10 pt-16">
              ...
              <div>
                 ...
                 <p className="text-lg md:text-xl text-white/40 mt-4 font-medium">{content?.footerAddressSub || 'Visit us for a coffee and a chat.'}</p>
              </div>
           </div>
           ...
           <div className="mt-32 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-12">
              ...
              <div className="flex flex-wrap justify-center gap-6 md:gap-12">
                 ...
                 <a href="/privacy" target="_blank" className="text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors">Privacy Policy</a>
                 <a href="/terms" target="_blank" className="text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors">Terms of Service</a>
              </div>
           </div>
        </motion.div>
      </div>
