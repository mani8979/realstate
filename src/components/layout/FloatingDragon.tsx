...
// ─────────────────────────────────────────────────────────────────────────────
// "Railway Track" Positioning Engine
//
//  Y-axis → pure scroll progress  (always drifts top→bottom as you scroll)
//  X-axis → section alignment     (switches side when a new section enters
//                                   the 25%–75% trigger band of the viewport)
//
//  The model holds its last X when no section is crossing the trigger band,
//  so it never snaps while the user is between sections.
// ─────────────────────────────────────────────────────────────────────────────

/** Scroll-driven Y: 15% → 80% of viewport height */
const getScrollY = (viewportH: number): number => {
  const pageH    = document.body.scrollHeight - viewportH;
  const progress = pageH > 0 ? Math.min(1, window.scrollY / pageH) : 0;
  return viewportH * (0.15 + progress * 0.65);
};

/**
 * Returns the target X based on which section is in the trigger band.
 * Returns `null` when no section is crossing → caller keeps lastX.
 *
 *  left-aligned text  → model RIGHT side
 *  right-aligned text → model LEFT  side
 *  center-aligned     → model RIGHT  side (default)
 */
const getScrollX = (
  viewportW : number,
  modelSize : number,
  ...
  for (const el of sections) {
    ...
    if (r.top < triggerBottom && r.bottom > triggerTop) {
      const align = el.dataset.modelAlign ?? 'left';
      return align === 'right'
        ? modelSize + pad          // text RIGHT → model LEFT
        : viewportW - modelSize - pad; // text LEFT/CENTER → model RIGHT
    }
  }

  return null; // no section in band → hold position
};

/**
 * Returns true when a .dragon-disappear or .model-hide-zone occupies
 * more than 25% of the viewport → fade model out.
 */
const checkShouldHide = (): boolean => {
  ...
  for (const zone of zones) {
    ...
    if (overlap > vH * 0.25) return true;
  }
  return false;
};
...
const FloatingDragon = () => {
  ...
  // ── Boot: lazy-load after 2.5 s or first scroll ─────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Start top-right
    let curX     = vW0 - ms0 - pad0;
    let curY     = vH0 * 0.15;
    let lastTargetX = curX; // holds last known X when between sections
    ...
    const update = (now: number) => {
      ...
      // ── Y: always follows scroll (never jumps) ─────────────────────────
      const targetY = getScrollY(vH);

      // ── X: section-driven, holds last value between sections ───────────
      const newX = getScrollX(vW, mSize);
      if (newX !== null) lastTargetX = newX;
      const targetX = lastTargetX;

      // ── Exponential lerp (frame-rate-independent) ──────────────────────
      // halfLifeX = 2.2 s → very smooth cross-screen glide
      // halfLifeY = 0.8 s → follows scroll naturally
      const alphaX = 1 - Math.pow(0.5, dt / 2.2);
      const alphaY = 1 - Math.pow(0.5, dt / 0.8);

      const dX = (targetX - curX) * alphaX;
      const dY = (targetY - curY) * alphaY;

      if (Math.abs(dX) > 0.3) curX += dX;
      if (Math.abs(dY) > 0.3) curY += dY;
  ...
  // ── Scroll-driven model camera orbit ────────────────────────────────────
  useMotionValueEvent(scrollYProgress, 'change', latest => {
    if (modelViewerRef.current) {
      modelViewerRef.current.cameraOrbit = `${latest * 360 * 3}deg 75deg 10m`;
    }
  });
  ...
  const modelW   = isMobile ? 110 : 170;
  const modelH   = isMobile ? 150 : 225;
    ...
    <>
      ...
      >
        <motion.div
          ...
          transition={{ duration: visible ? 1.0 : 0.25, ease: 'easeInOut' }}
        ...
        >
          {/* Subtle pulse ring — marks it as interactive */}
          <motion.div
            ...
            {hovered && (
              <motion.div
                ...
                className="absolute -top-12 left-1/2 -translate-x-1/2 whitespace-nowrap
                           bg-black/85 dark:bg-black/70 backdrop-blur-md
                           text-white text-[9px] font-black uppercase tracking-[0.18em]
                           px-4 py-2 rounded-full border border-primary/40
                           shadow-[0_0_16px_rgba(16,185,129,0.25)] pointer-events-none z-10"
              >
                    ...
                    {currentProperty.fruitDetails.map((detail: any, i: number) => (
                      <div key={i} className="space-y-6">
                        ...
                        {detail.isPointed ? (
                          ...
                          <p className="text-xl leading-relaxed font-medium bg-black/5 dark:bg-white/5 p-6 rounded-2xl border border-white/5 italic whitespace-pre-line">{detail.content}</p>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <div className="w-full md:w-1/2 h-64 md:h-full bg-white dark:bg-black/40 relative border-l border-white/5">
              {currentProperty?.threeDElement ? (
                <ModelViewer
                  ...
                  shadow-intensity="2" exposure="1.2"
                  style={{ width: '100%', height: '100%' }}
                ></ModelViewer>
